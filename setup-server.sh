...
if [ "$check_create_new_backend_app" = true ]; then
    ...
    sudo docker run --rm -v $(pwd)/backend:/backend -e $temp node:$DOCKER_NODE_VERSION \
        /bin/sh -c "cd backend && npm init -y && npm install --save-dev nodemon && npm set-script start \"node src/index.js\" && npm set-script test \"nodemon src/index.js\""
